...
out_log="${path_log}/out_pd_svc_remove_map_volume_${DateNow}_${random}.log"
error_log="${path_log}/error_pd_svc_remove_map_volume_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

#####################################################################################
#####                                                                           #####
#####                        SVC remove map volume                              #####
#####                                                                           #####
#####################################################################################
i=0
while [ $i -lt $host_len ] 
do
remove_map_info=$(ssh -i ${key_file} ${svc_user}@${svc_ip} "svctask rmvdiskhostmap -host ${host_id[$i]} $volume_id" 2> "${error_log}")
