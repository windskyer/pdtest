...
do
		case $volume_length in
				0)
				        volume_length=1;
				        volume_id=$param;;
				1)
				        volume_length=2;        
				        volume_expand_size=$param;;
		esac
done
...
out_log="${path_log}/out_pd_svc_expand_volume_${DateNow}_${random}.log"
error_log="${path_log}/error_pd_svc_expand_volume_${DateNow}_${random}.log"
...
#####                                                                           #####
#####################################################################################
volume_info=$(ssh -i ${key_file} ${svc_user}@${svc_ip} "svctask expandvdisksize -size $volume_expand_size -unit b $volume_id" 2> "${error_log}")
