...
out_log="${path_log}/out_get_tmp_info_v2.0_${DateNow}_${random}.log"
...
#####                                                                           #####
#####################################################################################
log_info $LINENO "get template info"
length=0
log_debug $LINENO "tmp_nm=${tmp_nm}"
if [ "$tmp_nm" == "" ]
then
	...
	do
		...
		log_debug $LINENO "tmp_info=${tmp_info}"
...
log_debug $LINENO "CMD:./ivm_unmount_nfs.sh \"${nfs_ip}|${nfs_path}|${tmp_path}\""
