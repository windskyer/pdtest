...
throwException "$error_result" "105068"
...
then
	result=$(mkdir ${tmp_path}"/"${tmp_name} 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
...
do
	...
	expect ./ssh.exp ${ivm_user} ${ivm_ip} "oem_setup_env|dd if=/dev/${disk_[$i]} of="${tmps_name}" bs=10M 2> ${error_log} > /dev/null &" > /dev/null 2>&1
	cp_size=0
	progress=18
	p=1
	...
	do
			sleep 15
			ps_rlt=$(ssh ${ivm_user}@${ivm_ip} "ps -ef|grep \"dd if=/dev/${disk_[$i]} of="${tmps_name}"\" | grep -v grep")
			if [ "${ps_rlt}" == "" ]
			then
			...
			cp_size=$(ls -l "${tmps_name}" | awk '{print $5/1024/1024}' 2> ${error_log})
			catchException "${error_log}"
			if [ "${error_result}" != "" ]
			then
				ssh ${ivm_user}@${ivm_ip} "kill $(ps -ef|grep \"dd if=/dev/${disk_[$i]} of="${tmps_name}"\" | grep -v grep | awk '{print $2}')" > /dev/null 2>&1
				ssh ${ivm_user}@${ivm_ip} "rm -f ${error_log}" >> $out_log 2>&1
				rm -Rf ${tmp_path}"/"${tmp_name} > /dev/null 2>&1
...
echo "id=$tmp_id" > $tmp_cfg
echo "files=${files#*,}" >> $tmp_cfg
echo "type=0" >> $tmp_cfg
echo "desc=$tmp_des" >> $tmp_cfg
