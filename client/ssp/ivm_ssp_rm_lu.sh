...
ssp_rm_lu() {
	...
	spname=$4
	lu_name=$5
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli rmbdsp -clustername ${clustername} -sp ${spname} -luudid ${lu_name}\""
	ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli rmbdsp -clustername ${clustername} -sp ${spname} -luudid ${lu_name}" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
...
rm_lu_param() {
	...
	if [ "$ivm_ip" == "" -o "$ivm_user" == "" -o "$clustername" == "" -o "$spname" == "" -o "$lu_name" == "" ]
	then
	...
	if [ $debug == 1 ]
	then
		echo ${ivm_ip} ${ivm_user} ${clustername} ${spname} ${lu_name}
	fi
...
ivm_ssp_rm_lu() {
	...
	ssp_rm_lu ${ivm_user} ${ivm_ip} ${clustername} ${spname} ${lu_name}
...
out_log="${path_log}/out_ivm_ssp_rm_lu_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_ssp_rm_lu_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_ssp_rm_lu $1
#ivm_ssp_rm_lu '172.30.126.23|padmin|testl|testssp|luudid'
