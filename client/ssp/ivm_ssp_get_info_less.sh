...
#	echo "\"pvUniqueId\":\"${cluster_metapvid}\",\c"
...
#	do
#	echo "{\c"
#	echo "\"luname\":\"${cluster_lu_name[$j]}\","
#	echo "\"luudid\":\"${cluster_lu_udid[$j]}\","
#	echo "\"provisiontype\":\"${cluster_lu_ProvisionType[$j]}\","
#	echo "\"size\":${cluster_lu_size[$j]},"
#	echo "\"unusedsize\":${cluster_lu_unused[$j]}"
#	echo "}\c"
#
#	j=$(expr $j + 1 ) 
#	
...
#	do
#	echo "{"
#	echo "\"hdiskName\":\"${cluster_pvname[$k]}\","
#	echo "\"pvUniqueId\":\"${cluster_pvudid[$k]}\","
#	echo "\"size\":${cluster_pvsize[$k]},"
#	echo "\"state\":\"${cluster_pvstate[$k]}\""
#	echo "}"
#	k=$(expr $k + 1 ) 
#	
#	if [ "$k" != "$cluster_pvnum" ]
...
#	echo -e "\"pvUniqueId\":\"${cluster_metapvid}\","
...
get_ip() {
	ivm_user=$1
        ivm_ip=$2
        nodename=$3
	nodeip=$(ssh ${ivm_user}@${ivm_ip} "cat /etc/hosts" | grep ${nodename} | awk '{print $1}' 2>/dev/null) 
	if [ "$(echo $?)" != "0" ]
	then
...
out_log="${path_log}/out_ivm_ssp_get_info_less_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_ssp_get_info_less_${DateNow}_${random}.log"
