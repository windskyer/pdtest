...
rm_clusternode() {
	...
	clustername=$3
	hostname=$4
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli cluster -rmnode -f -clustername ${clustername} -hostname ${hostname}\""
	addnode_ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli cluster -rmnode -f -clustername ${clustername} -hostname ${hostname}" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
		cluster_error "Rm cluster node error" "1000003"
	fi
...
ivm_ssp_alter_rm_node() {
	...
	do
		rm_clusternode  $ivm_ip $ivm_user $clustername ${vios_name[$i]}
		i=$(expr $i + 1 )
	done
}
...
out_log="${path_log}/out_ivm_ssp_alter_rm_node_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_ssp_alter_rm_node_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_ssp_alter_rm_node $1 $2
#ivm_ssp_alter_rm_node '172.30.126.23|padmin|testl' '172.30.126.24,P7R2-1-vios2'
