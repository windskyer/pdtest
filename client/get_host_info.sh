...
out_log="${path_log}/out_get_host_info_${DateNow}_${random}.log"
error_log="${path_log}/error_get_host_info_${DateNow}_${random}.log"
...
log_debug $LINENO "host_info=${host_info}"
log_debug $LINENO "CMD:exec ./ssh.exp ${ivm_user} ${ivm_ip} \"oem_setup_env|prtconf\" 2>&1"
ssh_out=$(exec ./ssh.exp ${ivm_user} ${ivm_ip} "oem_setup_env|prtconf" 2>&1)
log_debug $LINENO "ssh_out=${ssh_out}"
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"lssyscfg -r lpar -F lpar_id,name,lpar_env\""
