...
out_log="${path_log}/out_get_vm_info_v2.0_${DateNow}_${random}.log"
error_log="${path_log}/error_get_vm_info_v2.0_${DateNow}_${random}.log"
...
if [ "${lpar_name}" == "" ]
then
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"lssyscfg -r lpar -F name,lpar_id,lpar_env,state,os_version,default_profile,boot_mode,auto_start,uptime,rmc_state,auto_start\""
	...
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"lssyscfg -r prof -F lpar_id,os_type,min_mem,desired_mem,max_mem,proc_mode,min_proc_units,desired_proc_units,max_proc_units,min_procs,desired_procs,max_procs,sharing_mode,uncap_weight,virtual_scsi_adapters,virtual_eth_adapters\""
	...
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"lssyscfg -r lpar -F name,lpar_id,lpar_env,state,os_version,default_profile,boot_mode,auto_start,uptime,rmc_state,auto_start --filter lpar_names=\"${lpar_name}\"\""
	...
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"lssyscfg -r prof -F lpar_id,os_type,min_mem,desired_mem,max_mem,proc_mode,min_proc_units,desired_proc_units,max_proc_units,min_procs,desired_procs,max_procs,sharing_mode,uncap_weight,virtual_scsi_adapters,virtual_eth_adapters --filter lpar_names=\"${lpar_name}\"\""
...
log_debug $LINENO "vm_prof_info=${vm_prof_info}"
...
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsdev -type sea\" | grep Available | awk '{print \$1}'"
sea_name=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsdev -type sea" | grep Available | awk '{print $1}')
log_debug $LINENO "sea_name=${sea_name}"
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsmap -all -net -field svea physloc bdphysloc -fmt :\""
...
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsmap -all -type lv -field physloc backing -fmt :\" | grep -v ^$"
lv_map_info=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsmap -all -type lv -field physloc backing -fmt :" | grep -v ^$)
log_debug $LINENO "lv_map_info=${lv_map_info}"
#new add begin
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsmap -all -type disk -field physloc backing -fmt :\" | grep -v ^$"
pv_map_info=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsmap -all -type disk -field physloc backing -fmt :" | grep -v ^$)
log_debug $LINENO "pv_map_info=${pv_map_info}"
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsmap -all -type lv disk cl_disk -field physloc lun backing -fmt :\" | grep -v ^$"
...
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsmap -all -type cl_disk -field physloc backing -fmt :\" | grep -v ^$"
...
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsmap -all -field physloc svsa -fmt :\" | grep -v ^$"
vhost_slot_info=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsmap -all -field physloc svsa -fmt :" | grep -v ^$)
log_debug $LINENO "vhost_slot_info=${vhost_slot_info}"
...
log_debug $LINENO "cluster=${cluster}"
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} 2>/dev/null \"ioscli lssp -clustername $cluster|grep -E 'POOL_NAME'|awk '{print \$2}'\""
sp_name=$(ssh ${ivm_user}@${ivm_ip} 2>/dev/null "ioscli lssp -clustername $cluster|grep -E 'POOL_NAME'|awk '{print \$2}'")
log_debug $LINENO "sp_name=${sp_name}"
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} 2>/dev/null \"ioscli lssp -clustername $cluster -sp $sp_name -bd -field luname luudid size provisiontype unused -fmt :\""
...
log_debug $LINENO "lu_list_info=${lu_list_info}"
#new add end
