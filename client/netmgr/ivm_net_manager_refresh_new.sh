...
modify_ethernet_channel() {
	...
	ecinfo=$(ssh ${ivm_user}@${ivm_ip} "ioscli chdev -dev ${ecname} -attr backup_adapter=$backupeth mode=${mode} adapter_names=${eths}" 2>&1)
...
get_phyeth_info () {
	...
	ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsdev -fmt :|grep ent|grep -v EtherChannel|grep -v VLAN|grep -v Virtual|grep -v Share" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
	...
	do
		...
		ent4sea_mac[$i]=${ret##*.}
		enname=$(echo ${ent4seaname[$i]}|sed 's/t//g')
		ent4sea_ipAddr[$i]=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsdev -dev ${enname} -attr netaddr" | grep -v value | grep -v ^$)
...
get_veth_info () {
	...
	eth_info=$(ssh ${ivm_user}@${ivm_ip} "lssyscfg -r prof -F virtual_eth_adapters --filter lpar_ids=$vios_id" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
		pd_error "$ret" "1000609"
	fi
	
	hwres_info=$(ssh ${ivm_user}@${ivm_ip} "lshwres -r virtualio --rsubtype eth --level lpar --filter lpar_ids=$vios_id -F lpar_id,slot_num,is_trunk,trunk_priority" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
		pd_error "$hwres_info" "1000609"
	fi
	...
	do
		...
		then
			...
			vethvid[$j]=$(echo $ret | awk -F"/" '{print $4}')
			vethslot[$j]=$(echo $ret | awk -F"/" '{print $1}')
			#vethtruck[$j]=$(echo $ret | awk -F"/" '{print $5}')
			vethieee[$j]=$(echo $ret | awk -F"/" '{print $2}')
			vethisreq[$j]=$(echo $ret | awk -F"/" '{print $6}')
			vethtruck[$j]=$(echo "$hwres_info"|awk -F"," '{if($2==slotnum) {print $3}}' slotnum="${vethslot[$j]}")
			vethtrunkpriority[$j]=$(echo "$hwres_info"|awk -F"," '{if($2==slotnum) {print $4}}' slotnum="${vethslot[$j]}")
			j=$(expr $j + 1)
		else
			echo $ret|awk -F ',' '{for(i=1;i<=NF;i++) if($i)print $i}'|while read tmp
			do
				vethpvid[$j]=$(echo $tmp | awk -F"/" '{print $3}')
				vethvid[$j]=$(echo $tmp | awk -F"/" '{print $4}')
				vethslot[$j]=$(echo $tmp | awk -F"/" '{print $1}')
				#vethtruck[$j]=$(echo $tmp | awk -F"/" '{print $5}')
				vethieee[$j]=$(echo $tmp | awk -F"/" '{print $2}')
				vethisreq[$j]=$(echo $tmp | awk -F"/" '{print $6}')
				vethtruck[$j]=$(echo "$hwres_info"|awk -F"," '{if($2==slotnum) {print $3}}' slotnum="${vethslot[$j]}")
				vethtrunkpriority[$j]=$(echo "$hwres_info"|awk -F"," '{if($2==slotnum) {print $4}}' slotnum="${vethslot[$j]}")
				j=$(expr $j + 1)
			done
	...
	if [ "${ret}" != "" ]
		...
		do
			vent4seaname[$vent4seanumall]=$(echo $vent|awk -F':' '{print $1}')
			vent4seastatus[$vent4seanumall]=$(echo $vent|awk -F':' '{print $2}')
			vent4seatype[$vent4seanumall]=$(echo $vent|awk -F':' '{print $3}')
			
			vent4seanumall=$(expr $vent4seanumall + 1 )
		done
	#fi
		i=0
		while [ $i -lt $vent4seanumall ]
		do
			...
			while [ $j -lt $vethnum ]
			do
				if [ "${ventslotnum[$i]}" == "C${vethslot[$j]}" ]
				then
					veth4seapvid[$i]=${vethpvid[$j]}
					veth4seavid[$i]=${vethvid[$j]}
					veth4seatruck[$i]=${vethtruck[$j]}
					veth4seatruckpriority[$i]=${vethtrunkpriority[$j]}
					veth4vethieee[$i]=${vethieee[$j]}
					veth4vethisreq[$i]=${vethisreq[$j]}
				fi
				j=$(expr $j + 1)		
			done	
...
show_veth_info() {
	...
	j=0
	while [ $j -lt $vent4seanumall ]
	do
	echo  "{"
	echo  "\"trunkMode\":\"${veth4seatruck[$j]}\","
	echo  "\"trunkPriority\":\"${veth4seatruckpriority[$j]}\","
	echo  "\"ethName\":\"${vent4seaname[$j]}\","
	echo  "\"slotNum\":\"${ventslotnum[$j]}\","
	echo  "\"mac\":\"${vent4sea_mac[$j]}\","
	echo  "\"ieeeCompatible\":\"${veth4vethieee[$j]}\","
	echo  "\"isRequired\":\"${veth4vethisreq[$j]}\","
	echo  "\"locationCode\":\"${vent4sea_locationCode[$j]}\","
...
######################################
##comment
#ivm_net_manager_refresh() {
#	net_manager_refresh_param $1
#
#	net_manager_refresh ${ivm_ip} ${ivm_user}	
#}
#
#ivm_net_manager_refresh $1
##ivm_net_manager_refresh '172.30.126.12|padmin'
######################################


######################################
##new add begin: modify param format
