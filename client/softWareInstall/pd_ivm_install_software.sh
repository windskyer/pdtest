...
#####                                                                           #####
#####################################################################################
#echo "./exec_script.exp \"$ivm_user|$ivm_ip|$lpar_id|$lpar_user|$lpar_user_passwd|600\" \"$script_path\" \"yum_config.sh,yum_install_software.sh,$software_name\""
result=$(./exec_script.exp "$ivm_user|$ivm_ip|$lpar_id|$lpar_user|$lpar_user_passwd|600" "$script_path" "src_install_software.sh,$software_name,$script_path" 2>&1)
flag=$?
if [ $flag -ne 0 ]
then
	echo "result==$result"
