...
war_iso_name="war_"$DateNow"_"$random
...
if [ $? -ne 0 ]
then
	# rm -f $template_path/$war_path/$war_name > /dev/null 2>&1
	unmount_nfs
...
#####                                                                           #####
#####################################################################################
result=$(expect ./ssh.exp ${ivm_user} ${ivm_ip} "oem_setup_env|mkisofs -r -o ${cdrom_path}/$war_iso_name $template_path/$war_path/$war_name" 2>&1)
if [ $? -ne 0 ]
then
	# rm -f $template_path/$war_path/$war_name > /dev/null 2>&1
	unmount_nfs
...
#####                                                                           #####
#####################################################################################
mount_result=$(ssh ${ivm_user}@${ivm_ip} "ioscli loadopt -f -release -disk ${war_iso_name} -vtd ${vadapter_vcd}" 2>&1)
if [ $? -ne 0 ]
then
	# rm -f $template_path/$war_path/$war_name > /dev/null 2>&1
	expect ./ssh.exp ${ivm_user} ${ivm_ip} "oem_setup_env|rm -f ${cdrom_path}/$war_iso_name" > /dev/null 2>&1
	unmount_nfs
	echo "$mount_result" >&2
	exit 1
fi
...
#####                                                                           #####
#####################################################################################
result=$(./exec_script.exp "$ivm_user|$ivm_ip|$lpar_id|$lpar_user|$lpar_user_passwd|600" "$script_path" "liberty_deploy_apps.sh,$war_name,$server_name" 2>&1)
flag=$?
if [ $flag -ne 0 ]
then
	case $flag in
