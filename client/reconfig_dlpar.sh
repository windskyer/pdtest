...
out_log="${path_log}/out_reconfig_dlpar_${DateNow}_${random}.log"
error_log="${path_log}/error_reconfig_dlpar_${DateNow}_${random}.log"
...
then
	if [ "$rec_uncap_weight" != "" ]
	then
		log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"chhwres -r proc -o s -a uncap_weight=${rec_uncap_weight} --id ${lpar_id}\""
	...
	then
		log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"chsyscfg -r prof -i desired_proc_units=${rec_desired_proc_units},desired_procs=${rec_desired_proc},lpar_id=${lpar_id}\""
	...
	if [ "$rec_desired_proc" != "" ]
	then
		log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"chsyscfg -r prof -i desired_procs=${rec_desired_proc},lpar_id=${lpar_id}\""
...
if [ "$rec_desired_mem" != "" ]
then
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"chsyscfg -r prof -i desired_mem=${rec_desired_mem},lpar_id=${lpar_id}\""
