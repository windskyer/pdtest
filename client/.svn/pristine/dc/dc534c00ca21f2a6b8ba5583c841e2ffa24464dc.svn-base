...
lu_len=0
for param in $(echo $2 | awk -F"|" '{print $2}' | awk -F"," '{for(i=1;i<=NF;i++) print $i}')
do
	if [ "$param" != "" ]
	then
		lu_udid[$lu_len]=$(echo $param)
		lu_len=$(expr $lu_len + 1)
	fi
done
...
clustername=$(echo $2 | awk -F"|" '{print $1}' | awk -F"," '{print $1}')
spname=$(echo $2 | awk -F"|" '{print $1}' | awk -F"," '{print $2}')
src_hostname=$4
...
if [ "$src_hostname" == "" ]
then
	throwException "error ssp node hostname is null" "105053"
fi
...
out_log="${path_log}/out_ivm_vm_rebuild_lu_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_vm_rebuild_lu_${DateNow}_${random}.log"
...
while [ $i -lt $lu_len ]
do
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli mkbdsp -clustername ${clustername} -sp ${spname} -luudid ${lu_udid[$i]} -vadapter ${vadapter_vios}\""
	lu_map_info=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkbdsp -clustername ${clustername} -sp ${spname} -luudid ${lu_udid[$i]} -vadapter ${vadapter_vios}" 2> "${error_log}")
