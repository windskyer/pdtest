...
check_template(){
	...
	#temp_info=$(ssh ${ivm_user}@${ivm_ip} "ls $cache_path" 2> "${error_log}")
	#catchException "${error_log}"
	#if [ "$error_result" != "" ]
	#then
		#throwException "The cache path $cache_path dose not exist" "105009"
		#ssh ${ivm_user}@${ivm_ip} "mkdir $cache_path" > /dev/null 2>&1
	#fi
	
	tmp_img=$(ssh ${ivm_user}@${ivm_ip} "ls $cache_path" 2>&1)
	if [ $? -ne 0 ]
	then
		result=$(expect ./ssh.exp ${ivm_user} ${ivm_ip} "oem_setup_env| mkdir $cache_path| chmod 777 $cache_path|exit|exit" 2>&1)
	fi
	
	tmp_img=$(ssh ${ivm_user}@${ivm_ip} "cat $temp_path/$temp_name/$temp_name.cfg" 2>&1)
	if [ $? -ne 0 ]
	then
		throwException "$tmp_img" "105009"
	fi
	log_debug $LINENO "tmp_img=${tmp_img}"
	
	cache_img=$(ssh ${ivm_user}@${ivm_ip} "cat $cache_path/$temp_name/$temp_name.cfg" 2>&1)
	if [ $? -ne 0 ]
	then
		cache_cp=1
		return
	fi
	log_debug $LINENO "cache_img=${cache_img}"
	
	temp_len=0
	for line in $(echo "$tmp_img" | grep "^files=" | awk -F"=" '{print $2}' | awk -F"," '{for(i=1;i<=NF;i++) print $i}')
	do
		if [ "X$line" != "X" ]
		then
			tmp_file_name=$(echo $line | awk -F"|" '{print $1}')
			tmp_file_name=${tmp_file_name##*/}
			result=$(ssh ${ivm_user}@${ivm_ip} "ls -l $temp_path/$temp_name/$tmp_file_name" 2>&1)
			if [ $? -ne 0 ]
			then
				throwException "$result" "105009"
			fi
			temp_file[$temp_len]=$tmp_file_name
			temp_size[$temp_len]=$(echo "$result" | awk '{print $5/1024}')
		fi
		temp_len=$(expr $temp_len + 1)
	done
	
	
	cache_len=0
	for line in $(echo "$cache_img" | grep "^files=" | awk -F"=" '{print $2}' | awk -F"," '{for(i=1;i<=NF;i++) print $i}')
	do
		if [ "X$line" != "X" ]
		then
			tmp_file_name=$(echo $line | awk -F"|" '{print $1}')
			tmp_file_name=${tmp_file_name##*/}
			result=$(ssh ${ivm_user}@${ivm_ip} "ls -l $cache_path/$temp_name/$tmp_file_name" 2>&1)
			if [ $? -ne 0 ]
			then
				cache_cp=1
				return
			fi
			cache_file[$cache_len]=$tmp_file_name
			cache_size[$cache_len]=$(echo "$result" | awk '{print $5/1024}')
		fi
		cache_len=$(expr $cache_len + 1)
	done	
	
	i=0
	while [ $i -lt $cache_len ]
	do
		j=0
		is_exit=0
		while [ $j -lt $temp_len ]
		do
			if [ "${cache_file[$i]}" == "${temp_file[$j]}" ]
			#if [ $(ssh ${ivm_user}@${ivm_ip} "ls $cache_path" | grep "$temp_name" | wc -l | sed 's/ //g') -gt 0 ]
			then
				if [ "${cache_size[$i]}" != "${temp_size[$j]}" ]
				then
					cache_cp=1
					return
				fi
...
copy_check() {
	...
	progress=0
	i=1
	while [ ${cp_size} -lt ${file_size} ]
	do
		log_info $LINENO "sleep 10"
		sleep 10
		ps_rlt=$(ssh ${ivm_user}@${ivm_ip} "ps -ef | grep cp | grep \"${template_path}/${template_name}\" | grep -v grep" 2>&1)
		if [ "${ps_rlt}" == "" ]
		then
			dd_rlt=$(ssh ${ivm_user}@${ivm_ip} "cat ${error_log}" 2>&1)
		...
		cp_size=$(ssh ${ivm_user}@${ivm_ip} "ls -l ${cache_path}/${template_name}" 2>&1)
		if [ $? -ne 0 ]
		then
			ssh ${ivm_user}@${ivm_ip} "kill $(ps -ef | grep cp | grep \"${template_path}/${template_name}\" | grep -v grep | awk '{print $2}')" > /dev/null 2>&1
			throwException "$error_result" "105014"
		fi
		cp_size=$(echo "$cp_size" | awk '{array[$9]=$5/1024/1024; size=0; for(key in array) size=size+array[key]} END {print size}')
		if [ "$(echo ${cp_size}" "$(echo ${file_size} | awk '{printf "%0.2f",$1/5*i}' i="$i") | awk '{if($1>=$2) print 0}')" = "0" ]
		then
...
out_log="${path_log}/out_ivm_create_vm_cache_${lpar_name}_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_create_vm_cache_${lpar_name}_${DateNow}_${random}.log"
...
then
	is_mount=0
	template_path_now=$template_path
fi
...
then
	...
	#####                                                                           #####
	#####################################################################################
	log_info $LINENO "check cache"
	check_template $template_path $cache_path $template_name
	...
	if [ $cache_cp -eq 1 ]
	then
		file_size=$(ssh ${ivm_user}@${ivm_ip} "ls -l ${template_path}/${template_name}" 2>&1)
		if [ $? -ne 0 ]
		then
			throwException "$file_size" "105009"
		fi
		file_size=$(echo "$file_size" | awk '{array[$9]=$5/1024/1024; size=0; for(key in array) size=size+array[key]} END {print size}')
		# echo "file_size==$file_size"
		log_debug $LINENO "CMD:ssh.exp ${ivm_user} ${ivm_ip} \"oem_setup_env|cp -R $template_path/$template_name $cache_path\""
		result=$(expect ./ssh.exp ${ivm_user} ${ivm_ip} "oem_setup_env|cp -R $template_path/$template_name $cache_path > /dev/null 2> $error_log &")
		if [ $? -ne 0 ]
		then
			throwException "$result" "105009"
		fi
...
log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"cat ${template_path_now}/${template_name}/${template_name}.cfg\" | grep "files=" | awk -F"=" '{print $2}'"
img=$(ssh ${ivm_user}@${ivm_ip} "cat ${template_path_now}/${template_name}/${template_name}.cfg" 2> "${error_log}" | grep "files=" | awk -F"=" '{print $2}')
...
then
	...
	do
		...
		then
			...
			tmp_name=${tmp_name##*/}
			ssh ${ivm_user}@${ivm_ip} "ls ${template_path_now}/${template_name}/$tmp_name" > /dev/null 2> $error_log
			catchException "${error_log}"
			throwException "$error_result" "105009"
			img_[$i]=${template_path_now}/${template_name}/$tmp_name
			i=$(expr $i + 1)
		fi
	done
else
