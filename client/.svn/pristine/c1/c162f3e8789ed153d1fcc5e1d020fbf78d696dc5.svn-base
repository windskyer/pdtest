...
while [ $i -lt $host_len ] 
do
map_info=$(ssh -i ${key_file} ${svc_user}@${svc_ip} "svctask mkvdiskhostmap -force -host ${host_id[$i]}  $volume_id" 2> "${error_log}")
log_debug $LINENO "map_info=${map_info}"
catchException "${error_log}"
if [ "$error_result" != "" ]
then
	throwException "$error_result" "100000"
fi
scsi_id=$(echo "$map_info"|grep "successfully"|awk -F'[][]' '{print $2}')
if [ "$scsi_id" == "" ]
then
	throwException "map Volume to host ${host_id[$i]} in SVC failed" "100000"
fi
