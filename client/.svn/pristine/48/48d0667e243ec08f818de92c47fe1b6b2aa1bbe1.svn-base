...
add_clusternode() {
	...
	if [ "$(echo $?)" != "0" ]
	then
		cluster_error "Add cluster node error:${addnode_ret}" "1000002"
	fi
...
addnode_param() {
	...
	do
		...
		if [ "vios_ip[$vios_length]" == "" ]
		then
			cluster_error "param error" "1000000"
		fi
		
		vios_name[$vios_length]=$(echo $param | awk -F"," '{print $2}')
		if [ "vios_name[$vios_length]" == "" ]
		then
			cluster_error "param error" "1000000"
		fi
		...
		if [ $debug == 1 ]
		then
			echo ${vios_ip[$vios_length]} ${vios_name[$vios_length]}
		fi
...
ivm_ssp_alter_add_node() {
	...
	while [ $i -lt $vios_length ]
	do
		log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli cluster -status -clustername ${clustername} -fmt ':'\" | grep ${vios_name[$i]}"
		css=$(ssh ${ivm_user}@${ivm_ip} "ioscli cluster -status -clustername ${clustername} -fmt ':'" | grep ${vios_name[$i]})
		if [ "$(echo $?)" != "0" ]
		then
			cluster_error "Ssp get info error" "1000303"
		fi
		log_debug $LINENO "css=${css}"
		cluster_nodename[$i]=$(echo $css | awk -F ':' '{print $3}')

		cluster_nodeip[$i]=$(ssh ${ivm_user}@${ivm_ip} "cat /etc/hosts" | grep ${cluster_nodename[$i]} | awk '{print $1}'|head -1) 
		if [ "$(echo $?)" != "0" ]
		then
			cluster_error "Ssp get info error" "1000403"
		fi
		
		cluster_status[$i]=$(echo $css | awk -F ':' '{print $2}')
		cluster_nodestatus[$i]=$(echo $css | awk -F ':' '{print $6}')
		cluster_poolstatus[$i]=$(echo $css | awk -F ':' '{print $7}')
		cluster_nodemtm[$i]=$(echo $css | awk -F ':' '{print $4}')
		cluster_nodepn[$i]=$(echo $css | awk -F ':' '{print $5}')
		

		echo  "{"
		echo  "\"nodeIp\":\"${cluster_nodeip[$i]}\","
		echo  "\"nodeName\":\"${cluster_nodename[$i]}\","
		echo  "\"partitionNum\":${cluster_nodepn[$i]},"
		echo  "\"poolState\":\"${cluster_poolstatus[$i]}\","
		echo  "\"state\":\"${cluster_nodestatus[$i]}\""
		echo  "}"
...
out_log="${path_log}/out_ivm_ssp_alter_add_node_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_ssp_alter_add_node_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_ssp_alter_add_node $1 $2
#ivm_ssp_alter_add_node '172.30.126.23|padmin|testl' '172.30.126.24,P7R2-1-vios2'
