...
ssp_alter_lu() {
	...
	lu_name=$5
	lu_newname=$6
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli chbdsp -clustername ${clustername} -sp ${spname} -bd ${lu_name} -mv ${lu_newname}\""
	ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli chbdsp -clustername ${clustername} -sp ${spname} -bd ${lu_name} -mv ${lu_newname}" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
		cluster_error "Ssp alter lu error" "1000033"
	fi
...
alter_lu_param() {
	...
	if [ "$ivm_ip" == "" -o "$ivm_user" == "" -o "$clustername" == "" -o "$spname" == "" -o "$lu_name" == "" -o "$lu_newname"  == "" ]
	then
	...
	if [ $debug == 1 ]
	then
		echo ${ivm_ip} ${ivm_user} ${clustername} ${spname} ${lu_name} ${lu_newname}
	fi
...
ivm_ssp_alter_lu() {
	...
	ssp_alter_lu ${ivm_user} ${ivm_ip} ${clustername} ${spname} ${lu_name} ${lu_newname}
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lssp -clustername ${clustername} -sp ${spname} -bd -fmt ':'\"|grep ${lu_newname}"
	lu=$(ssh ${ivm_user}@${ivm_ip} "ioscli lssp -clustername ${clustername} -sp ${spname} -bd -fmt ':'"|grep ${lu_newname})
	if [ "$(echo $?)" != "0" ]
	then
		cluster_error "Ssp get info error" "1000603"
	fi
...
out_log="${path_log}/out_ivm_ssp_alter_lu_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_ssp_alter_lu_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_ssp_alter_lu $1
#ivm_ssp_alter_lu '172.30.126.12|padmin|testliuwei|sspzzy|luzzy'
