...
lvm_alter_lv() {
	...
	lvnewsize=$5
	chsize="$lvnewsize""M"
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli extendlv ${lvname} ${chsize}\""
	ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli extendlv ${lvname} ${chsize}" 2>&1)
	if [ "$(echo $?)" != "0" ]
	then
...
alter_lv_param() {
	...
	if [ $debug == 1 ]
	then
		echo $ivm_ip $ivm_user $vgname $lvname $lvnewsize
	fi

        oldlv_info=$(ssh ${ivm_user}@${ivm_ip} "ioscli lslv $lvname -field ppsize pps -fmt :" 2>&1)
        ...
        oldppsize=$(echo ${oldlv_info} | awk -F":" '{print $1}'|awk '{print $1}')
        oldpps=$(echo ${oldlv_info} | awk -F":" '{print $2}')
        oldlv_size=$(echo "$oldppsize*$oldpps"|bc)

	if [ $oldlv_size -gt $lvnewsize ]
	then
		pd_error "$oldlv_info" "100000"
	fi
	lvnewsize=$(expr $lvnewsize - $oldlv_size )
}
...
ivm_lvm_alter_lv() {
	...
	lvm_alter_lv ${ivm_user} ${ivm_ip} ${vgname} ${lvname} ${lvnewsize}
	...
	unique_ids=$(ssh "$ivm_user"@"$ivm_ip" "ioscli lslv -pv $lvname|grep -Ev 'N/A|DISTRIBUTION'|awk '{print \$1}'|while read pv;do ioscli lsdev -dev \$pv -attr unique_id|tail -1;done")
	unique_ids='["'`echo $unique_ids|sed 's/\ /\",\"/g'`'"]'
...
out_log="${path_log}/out_ivm_lvm_alter_lv_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_lvm_alter_lv_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_lvm_alter_lv $1 $2
#ivm_lvm_alter_lv '172.30.126.12|padmin|testvg' 'testlv|2048'
