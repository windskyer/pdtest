...
#####                                                                           #####
#####################################################################################
result=$(expect ./exec_script.exp "$ivm_user|$ivm_ip|$lpar_id|$lpar_user|$lpar_user_passwd|600" "$script_path" "liberty_get_state.sh,$server_name" 2>&1)
flag=$?
if [ $flag -ne 0 ]
then
	case $flag in
...
wlp_state=$(echo "$result" | grep "^wlp_state=" | awk -F"=" '{print $2}')

echo "{\"wlp_state\":\"$wlp_state\"}"