...
ssp_create_lu()  {
	...
	lu_mode=$7
	lpar_id=$8
if [ "${lu_mode}" == "" -o "${lu_mode}" == "thin" ]
then
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli mkbdsp -clustername ${clustername} -sp ${spname} ${lu_size} -bd ${lu_name}\""
	ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkbdsp -clustername ${clustername} -sp ${spname} ${lu_size} -bd ${lu_name}" 2>&1)
else
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli mkbdsp -clustername ${clustername} -sp ${spname} ${lu_size} -bd ${lu_name} -${lu_mode}\""
	...
	#get the udid
	echo $ret|grep -E "Udid"|awk -F ":" '{print $3}'
...
create_lu_param() {
	...
	#if [ "$ivm_ip" == "" -o "$ivm_user" == "" -o "$clustername" == "" -o "$lu_size" == "" -o "$spname" == "" -o "$lu_name" == "" -o "$lpar_id" == "" ]
	if [ "$ivm_ip" == "" -o "$ivm_user" == "" -o "$clustername" == "" -o "$lu_size" == "" -o "$spname" == "" -o "$lu_name" == "" ]
	then
	...
	if [ $debug == 1 ]
	then
		echo ${ivm_ip} ${ivm_user} ${clustername} ${lu_size} ${spname} ${lu_name} ${lu_mode} ${lpar_id}
	fi
...
ivm_ssp_create_lu() {
	...
	lu_udid=$(ssp_create_lu ${ivm_user} ${ivm_ip} ${clustername} ${spname} ${lu_size} ${lu_name} ${lu_mode} ${lpar_id})
	
	log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lssp -clustername ${clustername} -sp ${spname} -bd -fmt ':'\"|grep ${lu_udid}"
	lu=$(ssh ${ivm_user}@${ivm_ip} "ioscli lssp -clustername ${clustername} -sp ${spname} -bd -fmt ':'"|grep ${lu_udid})
	if [ "$(echo $?)" != "0" ]
	then
		cluster_error "Ssp get info error" "1000603"
	fi
	...
	then
		...
		i=0
		echo "{\"luname\":\"${cluster_lu_name[$i]}\",\"luudid\":\"${cluster_lu_udid[$i]}\",\"provisiontype\":\"${cluster_lu_ProvisionType[$i]}\",\"size\":${cluster_lu_size[$i]},\"unusedsize\":${cluster_lu_unused[$i]}}"
	fi
}
...
out_log="${path_log}/out_ivm_ssp_create_lu_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_ssp_create_lu_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_ssp_create_lu $1
#ivm_ssp_create_lu '172.30.126.23|padmin|testl|testssp|2000|luzzy|thick'
