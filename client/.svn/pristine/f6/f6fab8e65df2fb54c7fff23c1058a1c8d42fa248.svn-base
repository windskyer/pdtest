...
#		ret=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsvg |grep ${vg_name}" 2>&1)
	...
	then
		...
		do
			if [ "$vg" == "rootvg" ]
			then
				log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsvg $vg -field vgstate vgid maxlvs numlvs totalpvs totalpps freepps usedpps pppervg maxpvs ppperpv -fmt :\" | head -n 1"                 
				...
				log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsvg $vg -field vgstate vgid maxlvs numlvs totalpvs totalpps freepps usedpps pppervg maxpvs -fmt :\" | head -n 1" 
			...
			log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsvg -lv $vg -field lvname -fmt :\"" 
			...
			then
				...
				do
					...
					log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lslv $lv -field lvname maxlps ppsize pps -fmt :\"" 
					lv_info[$vg_length]=$(ssh ${ivm_user}@${ivm_ip} "ioscli lslv $lv -field lvname maxlps ppsize pps -fmt :" 2> /dev/null)"|"${lv_info[$vg_length]}
					log_debug $LINENO "lv_info=${lv_info[$vg_length]}"
...
out_log="${path_log}/out_ivm_vgm_get_info_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_vgm_get_info_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_lvm_get_info $1
#ivm_lvm_get_info '172.30.126.12|padmin'
