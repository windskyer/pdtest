...
modify_sea_param () {
	...
	if [ "$ivm_ip" == "" -o "$ivm_user" == "" -o "$seaname" == "" -o "$td" == "" -o "$vea" == "" -o "$dvea" == "" -o "$pvid" == "" ]
	then
	...
	if [ $debug == 1 ]
	then
		echo $ivm_ip $ivm_user $seaname $td ${vea} $dvea ${pvid}
	fi
...
ivm_modify_sea() {
	...
	modify_sea ${ivm_ip} ${ivm_user} ${seaname} ${td} "${vea}" ${dvea} ${pvid}	
}
...
ivm_modify_sea $1
#ivm_modify_sea '172.30.126.12|padmin|ent9|ent2|ent5,ent6|ent5|1'
