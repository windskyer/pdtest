...
create_ethernet_channel () {
	...
	log_debug $LINENO "nowioslevel=${nowioslevel}"
	#echo $nowioslevel
	check_ret=""
	check_ret=$(awk 'BEGIN{split ('\"$nowioslevel\.$ioslevel\"',v,".");for(i=1;i<=4;i++)if('"v[i]"'>'"v[4+i]"') {print 1;break;} else if('"v[i]"'<'"v[4+i]"') {print 2;break} }')
	#echo $check_ret
	log_debug $LINENO "check_ret=${check_ret}"
	if [ "$cmdinfo" == "" ]
	then
		if [ "${check_ret}" == "2" ]
		then
			#ioslevel is lower than 2.2.3.1
			ecinfo=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkvdev -lnagg ${eths}")
		else
			#ioslevel is higher than 2.2.3.1
			eths=$(echo "$eths" | sed 's/,/ /')
			ecinfo=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkvdev -lnagg ${eths}")
			#echo $ecinfo
		fi
		...
		then
			#ioslevel is lower than 2.2.3.1
			ecinfo=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkvdev -lnagg ${eths} -attr ${cmdinfo}")
		else
			#ioslevel is higher than 2.2.3.1
			eths=$(echo "$eths" | sed 's/,/ /')
			ecinfo=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkvdev -lnagg ${eths} -attr ${cmdinfo}")
...
create_ethernet_channel_param () {
	...
	if [ "$ivm_ip" == "" -o "$ivm_user" == "" -o "$mode" == "" -o "$eths" == "" ]
	then
	...
	if [ $debug == 1 ]
	then
		echo $ivm_ip $ivm_user ${mode} $backupeth ${eths}
	fi

}


ivm_create_ethernet_channel() {
	
	create_ethernet_channel_param $1

	create_ethernet_channel $ivm_ip $ivm_user ${mode} $backupeth ${eths}
}
...
out_log="${path_log}/out_ivm_create_ethernet_channel_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_create_ethernet_channel_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_create_ethernet_channel $1
#ivm_create_ethernet_channel '172.30.126.12|padmin|8023ad|ent5|ent6'
