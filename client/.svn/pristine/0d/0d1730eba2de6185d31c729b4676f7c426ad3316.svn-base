#!/usr/bin/ksh
#paras:    ./reconfig_dlpar.sh "ip|user|id|proc_units|proc|mem|weight|share_mode" "lv_name1,rec_stosize1|lv_name2,rec_stosize2..." "vg_name1,lv_size1|vg_name2,lv_size2... or pv_name1|pv_name2..."

#example1: ./reconfig_dlpar.sh "172.30.126.13|padmin|4|0.2|2|1024|128|" "lv02,10496"
#example2: ./reconfig_dlpar.sh "172.30.126.13|padmin|5|0.2|2|1024|128|" "aix1_lv1,2048|aix1_lv2,2048"
#example3: ./reconfig_dlpar.sh "172.30.126.13|padmin|5|0.2|2|1024|128|" "aix1_lv1,2048|aix1_lv2,2048" "rootvg,512"
#example4: ./reconfig_dlpar.sh "172.30.126.13|padmin|5|0.2|2|1024|128|" "aix1_lv1,2048|aix1_lv2,2048" "hdisk2"

#./reconfig_add_storage.sh "172.30.126.13|padmin|5" "hdisk2"

#add disk(pv,lv),extend lv,
#./$0 $1 $2 $3:abstract rec_stosize from $1,then add it into $2s,$2 is 'lv_name1,rec_stosize1|...' 'vg_name,lv_size|pv_name'
# refer to $3 so as to differ pv or lv:2 pars. are lv, 1 par. is pv.
...
then
    ...
    do
       new_lv_name[$i]=$(ssh ${ivm_user}@${ivm_ip} "ioscli mklv ${new_vg_name[$i]} ${new_lv_size[$i]}M ")
       
       ######################################################################################
       ######                                                                           #####
       ######                          create virtual vtscsi :lv                        #####
       ######                                                                           #####
       ######################################################################################
       echo "$(date) : create virtual cdrom" >> $out_log
       vadapter_vcd=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkvdev -vdev ${new_lv_name[$i]} -vadapter ${vadapter_vios}" 2> "${error_log}" | awk '{print $1}')
       catchException "${error_log}"
...
#    echo "extend pv"
    ...
    do
       ...
       ######################################################################################
       ######                                                                           #####
       ######                          create virtual vtscsi :pv                        #####
       ######                                                                           #####
       ######################################################################################
       echo "$(date) : create virtual vtscsi" >> $out_log
       vtscsi_ok=$(ssh ${ivm_user}@${ivm_ip} "ioscli lsmap -all -type disk -field backing -fmt : | grep -v ^$ ")
       if [ "$vtscsi_ok" == "" ]
       then
            vadapter_vcd=$(ssh ${ivm_user}@${ivm_ip} "ioscli mkvdev -vdev ${new_pv_name[$i]} -vadapter ${vadapter_vios}" 2> "${error_log}" | awk '{print $1}')
            catchException "${error_log}"
            throwException "$error_result" "105017" 
       fi      
