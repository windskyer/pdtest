...
show_veth_info() {
	...
	do
	echo -e "{"
	echo -e "\"bridgeType\":\"${veth4seatruck[$j]}\","
	echo -e "\"ethName\":\"${vent4seaname[$j]}\","
	echo -e "\"locationCode\":\"${vent4sea_locationCode[$j]}\","
...
show_lnagg_info() {
	...
	echo ${memberEth[$j]} | awk -F"," '{for(i=1;i<=NF;i++) print $i}' | while read eth
	do
		echo -e "{"
		echo -e "\"ethName\":\"$eth\""
		echo -e "}"
...
ivm_delete_ethernet_channel() {

	delete_ethernet_channel_param $1
	delete_ethernet_channel $ivm_ip $ivm_user $devname
}
...
out_log="${path_log}/out_ivm_delete_ethernet_channel_${DateNow}_${random}.log"
error_log="${path_log}/error_ivm_delete_ethernet_channel_${DateNow}_${random}.log"

log_debug $LINENO "$0 $*"

ivm_delete_ethernet_channel $1
#ivm_delete_ethernet_channel '172.30.126.12|padmin|ent9'
