...
out_log="${path_log}/out_ivm_vm_ref_eth_${DateNow}_${random}.log"
...
do
	...
	then
		...
		log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"ioscli lsdev -dev $sea -attr\""
				...
				then
					...
					do
						...
						then
							...
							# echo "slot==$slot"
							log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"lshwres -r virtualio --rsubtype eth --level lpar --filter lpar_ids=$vios_id,slots=$slot -F port_vlan_id,addl_vlan_ids\""
							vlans=$(ssh ${ivm_user}@${ivm_ip} "lshwres -r virtualio --rsubtype eth --level lpar --filter lpar_ids=$vios_id,slots=$slot -F port_vlan_id,addl_vlan_ids" | sed 's/,none//g' | sed 's/"//g')","$vlans
...
eth_info=$(ssh ${ivm_user}@${ivm_ip} "lssyscfg -r prof -F virtual_eth_adapters --filter lpar_ids=${lpar_id}" 2> $error_log | sed 's/"//g' | awk -F"," '{for(i=1;i<=NF;i++) print $i}' | awk -F"/" '{if($1==slot_num) { printf $1"/"$2"/"vlan_id"/"$4"/"$5"/"$6","; } else { printf $0","; } }' slot_num="$slot_num" vlan_id="$vlan_id")
...
if [ "$eth_info" == "" ]
then
	throwException "The virtual network info is null." "105080"
fi
...
then
	...
	catchException "${error_log}"
	throwException "$error_result" "105082"
else
	...
	then
		...
		catchException "${error_log}"
		throwException "$error_result" "105082"
		log_debug $LINENO "CMD:ssh ${ivm_user}@${ivm_ip} \"chhwres -r virtualio --rsubtype eth -o a -s $slot_num --id $lpar_id -a ieee_virtual_eth=0,port_vlan_id=$vlan_id,is_trunk=0\""
		ssh ${ivm_user}@${ivm_ip} "chhwres -r virtualio --rsubtype eth -o a -s $slot_num --id $lpar_id -a ieee_virtual_eth=0,port_vlan_id=$vlan_id,is_trunk=0" > /dev/null 2> $error_log
		catchException "${error_log}"
		throwException "$error_result" "105082"
	else
		throwException "Lpar does not support dynamic modification, please shutdown first." "105068"
